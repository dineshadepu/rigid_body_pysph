...
from rigid_body_pysph.math import (normalize_R_orientation, find_transpose)
from rigid_body_pysph.compute_rigid_body_properties import (
...
from pysph_dem.rigid_body.boundary_particles import (get_boundary_identification_etvf_equations,
...
def setup_rigid_body(pa, dim, total_no_of_walls=3):
    ...
    consts = {
        ...
        'izz':
        np.zeros(nb, dtype=float),
        # rotation angle (this is only for 2d)
        'rotation_angle':
        np.zeros(nb, dtype=float),
...
class GTVFRigidBody3DQuatMasterStep(IntegratorStep):
    """
    ...
    def stage2(self, d_idx, d_u, d_v, d_w, d_m,
               ...
               d_R,
               dt):
        didx9, didx4, i = declare('int', 3)
        R, R_t, R_dot, omega_mat = declare('matrix(9)', 4)
        q, q_dot = declare('matrix(4)', 2)
        tmp_moi_inv, new_moi, R_moi = declare('matrix(9)', 3)
        a1, a2, a3, b1, b2, b3 = declare('matrix(3)', 6)
        didx9 = 9 * d_idx
        didx4 = 4 * d_idx
        ...
        # Rate of change of orientation is matrix multiply omega mat with R

        # TODO:
        # test this, with proper multiplication (this is taken from equation 8
        # of The Quaternions with an application to Rigid Body Dynamic)
        q_dot[0] = 0.5 * (- w1 * q[1] - w2 * q[2] - w3 * q[3])
        q_dot[1] = 0.5 * (w1 * q[0] - w3 * q[2] + w2 * q[3])
        q_dot[2] = 0.5 * (w2 * q[0] + w3 * q[1] - w1 * q[3])
        q_dot[3] = 0.5 * (w3 * q[0] - w2 * q[1] + w1 * q[2])
        ...
        q[4] = q[3] / q_norm
        # =========================================================
        # normalize the orientation ends
        # =========================================================
        # copy back to particle array
        for i in range(4):
            d_q[didx4 + i] = q[i]
...
class RigidBody3DScheme(Scheme):
    def __init__(self, rigid_bodies_master, rigid_bodies_slave, boundaries, dim,
        ...
        # rigid body
        self.rb_evolve = "3d_rot"

        self.solver = None

    def add_user_options(self, group):
        choices = ['2d', '3d_rot', '3d_quat']
        group.add_argument("--rb-evolve",
                           action="store",
                           dest='rb_evolve',
                           default="2d",
                           choices=choices,
                           help="RB dynamics evolution (2d or 3d) " % choices)
        ...
        group.add_argument("--rb-2d", action="store",
                           dest="rb_2d", default=0.0,
                           type=float,
                           help="Surface energy")

    def consume_user_options(self, options):
        _vars = ['kr', 'kf', 'fric_coeff', 'en', 'gamma', 'rb_evolve']
    ...
    def _get_gtvf_equations(self):
        ...
        #############################################################
        # Compute the forces on the rigid bodies due to interaction #
        #############################################################
        # g5 = []
        # for name in self.rigid_bodies_master:
        #     g5.append(
    ...
    def configure_solver(self,
        ...
        if self.rb_evolve == "2d":
            bodystep = GTVFRigidBody2DMasterStep()
        elif self.rb_evolve == "3d_quat":
            bodystep = GTVFRigidBody3DQuatMasterStep()
